    ...
    <div className="w-full" {...getRootProps()}>
      ...
      >
        <input
          ...
          className="hidden"
          {...getInputProps()} // Ensure inputProps from useDropzone are included
        />
        <div className="absolute inset-0 [mask-image:radial-gradient(ellipse_at_center,white,transparent)]">
          <GridPattern />
        </div>
        <div className="flex flex-col items-center justify-center">
          <p className="relative z-10 font-sans font-bold text-neutral-300 text-3xl">
            Upload your resume
          </p>
          <p className="relative z-20 font-sans font-normal text-neutral-400 text-base mt-2">
            Drag or drop your files here or click to upload
          </p>
          <div className="relative w-full max-w- mt-10 mx-auto">
            {files.length > 0 &&
              files.map((file, idx) => (
                <motion.div
                  key={"file" + idx}
                  ...
                  className={cn(
                    "relative overflow-hidden z-40 bg-neutral-900 flex flex-col items-center justify-center md:h-24 p-4 mt-4 w-full mx-auto rounded-md",
                    "shadow-sm"
                  )}
                >
                  <div className="flex justify-center items-center w-full gap-4">
                    <motion.p
                      ...
                      animate={{ opacity: 1 }}
                      layout
                      className="text-base text-neutral-200 text-center w-full"
                    >
                      {file.name}
                    </motion.p>
                    <motion.p
                      ...
                      animate={{ opacity: 1 }}
                      layout
                      className="rounded-lg px-2 py-1 text-base text-neutral-600 bg-neutral-800 text-white shadow-input text-center w-full"
                    >
